...
/**
zend_class_entry是内核中定义的一个结构体，是内核实现PHP语言中类与对象的一个非常基础、关键的结构类型。
他就相当于我们定义的类的原型。
如果我们想获得一个名字为myclass的类该怎么做呢？首先我们定义一个zend_class_entry变量，并为它设置名字，
最后注册到runtime中去。
*/
...
/**
ZEND_BEGIN_ARG_INFO_EX is a PHP 5 addition is used for producing cleaner code,
enabling type hinting, pass-by-reference and reflection.
Consider the following arginfo declarations for your actual function that just returns true:
...
/**
yaf_application类的方法
类方法的定义使用PHP_ME宏,在h文件中使用ZEND_METHOD声明
*/
...
zend_function_entry yaf_application_methods[] = {
    /**
    ZEND_ACC_CTOR它的作用便是声明这个方法是此类的构造函数,
    而 ZEND_ACC_PUBLIC|ZEND_ACC_CTOR 是我们常见的掩码或运算，代表它是一个public 类型的构造函数
    */
...
相当于
ZEND_MINIT_FUNCTION(yaf_application)
也即注册类application和方法
这样，我们在PHP中可以使用Yaf\Applicatioin这个类了：
...
YAF_STARTUP_FUNCTION(application) {
	...
	*/
    ...
    #define ZEND_STRL(str) (str), (sizeof(str)-1)
    zend_declare_property_null(yaf_application_ce,"config",sizeof("config")-1,ZEND_ACC_PROTECTED)
    */
